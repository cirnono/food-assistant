...
LOAD_JS = r"""async function load() {
  try {
    const active = await api(
      '/api/v1/cooking-sessions/active?owner=household'
    );
    ...
    if (active) {
      ...
      document.querySelector('#idle').hidden = false;

      const state = await api(
        '/api/v1/home-assistant/state?owner=household'
      );
      ...
      document.querySelector('#start').disabled = !selected;
    }
  } catch (error) {
    fail(error);
  }
...
async function finishCooking(){{if(!confirm('确认完成烹饪并写入做饭历史吗？'))return;try{{const done=await api(`/api/v1/cooking-sessions/${{session.id}}/finish`,{{method:'POST',body:body({{select_next:true}})}});session=null;await load();const review=done.consumption_review;document.querySelector('#recommendation').innerHTML=`<strong>本次库存消耗等待确认</strong> · <a href="/consumption${{review?'#review-'+review.id:''}}">进入消耗确认</a>`}}catch(e){{fail(e)}}}}
